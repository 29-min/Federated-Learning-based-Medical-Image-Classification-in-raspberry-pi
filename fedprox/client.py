"""
FedProx 연합학습 클라이언트 (Dirichlet 기반 Non-IID)
PathMNIST (9개 클래스)를 Dirichlet Distribution으로 불균등 분할

FedProx 특징:
- Proximal Term을 추가하여 로컬 모델이 글로벌 모델에서 너무 멀어지는 것을 방지
- Loss = Original Loss + (mu/2) * ||w - w_global||^2
- Non-IID 환경에서 FedAvg보다 더 안정적인 수렴
"""
...
def train_fedprox(model, train_loader, global_model, epochs=5, lr=0.01, mu=0.01):
    """
    FedProx 학습 함수
    ...
    Args:
        ...
        global_model: 글로벌 모델 (proximal term 계산용)
        epochs: 에포크 수
        lr: 학습률
        mu: proximal term 계산 시 사용되는 계수 (클수록 글로벌 모델에 가깝게 유지)
            - mu=0: FedAvg와 동일
            - mu=0.01: 약한 regularization
            - mu=0.1: 중간 regularization
            - mu=1.0: 강한 regularization
    
    Returns:
        avg_loss, avg_acc, avg_f1: 학습 결과
    """
    # 클래스 가중치 계산
    ...
    # 글로벌 모델 파라미터 저장 (proximal term 계산용)
    ...
    for epoch in range(epochs):
        ...
        epoch_prox_loss = running_proximal_loss / len(train_loader)
        epoch_acc = 100. * correct / total
        
        # F1 score 계산 (macro average)
        epoch_f1 = f1_score(all_targets, all_predictions, average='macro', zero_division=0)
        
        print(f"  Epoch {epoch+1}/{epochs}: Loss={epoch_loss:.4f}, Prox={epoch_prox_loss:.4f}, Acc={epoch_acc:.2f}%, F1={epoch_f1:.4f}")
...
class FlowerClientFedProx(fl.client.NumPyClient):
    """Flower 클라이언트 (FedProx 버전, 체크포인트 기능 추가)"""
    
    def __init__(self, client_id, model, train_loader, test_loader, local_epochs, lr, mu):
        ...
        self.lr = lr
        self.mu = mu  # FedProx의 proximal term 계수
        self.checkpoint_dir = f"checkpoints_fedprox/client_{client_id}"
    ...
    def load_checkpoint(self):
        ...
        if checkpoints:
            ...
            print(f"🆕 새로운 학습 시작 (FedProx, mu={self.mu})")
    ...
    def fit(self, parameters, config):
        ...
        print(f"\n{'='*60}")
        print(f"클라이언트 {self.client_id} - 라운드 {self.current_round} 학습 시작 (FedProx)")
        print(f"  mu={self.mu} (proximal term 계수)")
        print(f"{'='*60}")
        
        # 글로벌 모델 파라미터를 받아서 설정
        self.set_parameters(parameters)
        
        # 글로벌 모델 복사 (proximal term 계산용)
        global_model = copy.deepcopy(self.model)
        
        # FedProx 학습
        train_loss, train_acc, train_f1 = train_fedprox(
            self.model,
            self.train_loader,
            global_model,
...
def main():
    """클라이언트 메인 함수"""
    parser = argparse.ArgumentParser(description='연합학습 클라이언트 (FedProx)')
    ...
    parser.add_argument('--mu', type=float, default=0.01, help='FedProx mu (proximal term 계수)')
    
    args = parser.parse_args()
    
    print("\n" + "="*60)
    print(f"연합학습 클라이언트 {args.client_id} (FedProx)")
    print("="*60)
    print(f"서버: {args.server_address}")
    print(f"전체 클라이언트: {args.total_clients}")
    print(f"모델: SimpleCNN")
    print(f"알고리즘: FedProx")
    print(f"  ※ mu={args.mu} (0이면 FedAvg와 동일)")
