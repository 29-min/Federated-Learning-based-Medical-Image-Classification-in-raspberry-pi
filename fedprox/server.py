"""
FedProx 연합학습 서버 - 글로벌 평가 기능 포함
목표: PathMNIST 전체 테스트셋으로 글로벌 모델 성능 평가 (FedProx 방식)

FedProx vs FedAvg:
- 서버 측 집계 방식은 동일 (FedAvg 사용)
- 클라이언트가 학습 시 proximal term을 추가하여 안정성 향상
- Non-IID 환경에서 더 나은 성능 기대
"""
...
def evaluate_global_model(model, test_loader, device, round_num, num_classes=9):
    ...
    # 결과 출력
    print(f"\n{'='*70}")
    print(f"🌍 ROUND {round_num} - GLOBAL MODEL EVALUATION (FedProx)")
    print(f"{'='*70}")
...
def save_server_checkpoint(model, round_num, checkpoint_dir="checkpoints_fedprox/server"):
    """서버 체크포인트 저장"""
...
def load_server_checkpoint(model, checkpoint_dir="checkpoints_fedprox/server"):
    """서버 체크포인트 로드"""
    import glob
    ...
    if checkpoints:
        ...
        print(f"🆕 새로운 서버 학습 시작 (FedProx)")
        return 0


class GlobalEvaluationStrategyFedProx(FedAvg):
    """
    글로벌 평가 기능이 추가된 FedProx 전략
    - 매 라운드마다 글로벌 테스트셋으로 평가
    - 결과를 CSV/JSON으로 저장
    - 서버 측 집계는 FedAvg와 동일 (클라이언트가 proximal term 사용)
    """
    ...
    def __init__(
        ...
        global_test_loader,
        num_classes=9,
        checkpoint_dir="checkpoints_fedprox/server",
        results_dir="results_fedprox",
        **kwargs
    ):
        ...
        print(f"\n🔧 GlobalEvaluationStrategyFedProx 초기화")
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        3. 결과 저장
        """
        # FedAvg 기본 집계 (FedProx는 클라이언트 측에서만 다름)
...
def print_final_summary(results_dir="results_fedprox"):
    """최종 결과 요약 출력"""
...
def main():
    """서버 메인 함수"""
    parser = argparse.ArgumentParser(description='FedProx 연합학습 서버 (글로벌 평가)')
    ...
    parser.add_argument('--mu', type=float, default=0.01, help='FedProx mu (참고용)')
    ...
    print("\n" + "="*70)
    print("🌍 FedProx 연합학습 서버 - 글로벌 평가 기능")
    print("="*70)
    ...
    config = {
        ...
        'strategy': f'FedProx (mu={args.mu}) + Global Evaluation'
    }
    ...
    # 글로벌 평가 전략 생성 (FedProx)
    strategy = GlobalEvaluationStrategyFedProx(
        model=model,
    ...
    for i in range(config['num_clients']):
        print(f"  터미널 {i+1}:")
        print(f"  python3 client_fedprox.py --client_id {i} --total_clients {config['num_clients']} --alpha {args.alpha} --mu {args.mu}")
        print()
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print(f"✅ 연합학습 완료! (FedProx)")
        print(f"{'='*70}")
        print(f"\n📁 저장된 파일:")
        print(f"  - 체크포인트: checkpoints_fedprox/server/")
        print(f"  - 글로벌 평가 결과: results_fedprox/global_evaluation_results.csv")
        print(f"  - 상세 결과: results_fedprox/global_evaluation_results.json")
        print(f"  - 클라이언트 메트릭: results_fedprox/client_metrics_round_*.json")
        
        print(f"\n💡 다음 단계:")
        print(f"  1. results_fedprox/global_evaluation_results.csv로 시각화")
        print(f"  2. FedAvg vs FedProx 성능 비교")
        print(f"  3. 다양한 mu 값으로 실험")
        print(f"{'='*70}\n")
